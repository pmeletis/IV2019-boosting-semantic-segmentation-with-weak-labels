...
a tf.data.Dataset is generated from the {train, val}-imageid2bboxes.json, using
a python generator. The Python generator emmits imageid s and a list of (class, bbox_coords).
...
mid2cid = {'/m/0199g': 0,   # bicycle
           ...
           '/m/01bl7v': 6,  # boy
           '/m/05r655': 6,  # girl
           '/m/015qff': 7,  # traffic light
           '/m/01mqdt': 8,  # traffic sign
           '/m/02pv19': 8,  # stop sign
           'void': 9,
          }

def _gt_generator(filepath):
  ...
  for imageid, bboxes in imageid2bboxes.items():
    # convert grayscale and RGBA to RGB and then make range in [0.0, 1.0)
    rim = Image.open(op.join(PATH_train, imageid + '.jpg')).convert(mode='RGB')
    rim = np.array(rim, dtype=np.float32) / 256
    rla = np.zeros((*rim.shape[:2], len(mid2cid)), dtype=np.float32)
    for mid, coords_normalized in bboxes:
    ...
    rla = np.where(reduced_sum > 0.5,
                   rla / reduced_sum,
                   np.concatenate([np.zeros((len(mid2cid)-1), dtype=np.float32), (1.0,)]))
    assert np.all(np.abs(np.sum(rla, axis=2) - np.ones_like(rla)[..., -1]) < 0.001), (
        f'some pixels in rla for imageid {imageid} doesn\'t represent a multinomial distribution.')
    yield rim, rla, imageid.encode('utf-8')

def _train_prebatch_processing(rim, rla, imageid, params):
...
def prebatch_dataset(config, params):
  dataset = tf.data.Dataset.from_generator(
      functools.partial(_gt_generator, FILEPATH_train_imageid2bboxes),
